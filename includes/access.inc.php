...
function userIsLoggedIn()
{
    if (isset($_POST['action']) and $_POST['action'] == 'login') 
    {
            ...
            $password = md5($_POST['password'] . 'ankietavilo');
...
function dbContainsAuthor($name, $password) {
    ...
    try {
        $sql = 'SELECT COUNT(*) FROM users WHERE login = :name AND password = :password';
        $s = $pdo->prepare($sql);
...
function userIsAdmin() {
   include $_SERVER['DOCUMENT_ROOT'] . '/includes/dbconnect.inc.php';
    try {
        $sql = 'SELECT adminPrivileges FROM users WHERE login = :name';
        $s = $pdo->prepare($sql);
